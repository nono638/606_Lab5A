...
ggplot(global_monitor, aes(x = scientist_work, fill = scientist_work)) +
  geom_bar() +
  labs(
    x = "", y = "",
...
## Answer 1:
```{r Answer1, results = TRUE}
samp1 %>%
  count(scientist_work) %>%
  mutate(proportionOfSample = n /sum(n))

ggplot(samp1, aes(x = scientist_work, fill = scientist_work)) +
  geom_bar() +
  labs(
    x = "", y = "",
    title = "Samp1"
...
## Answer 2:
Given that we each set the seed at the start of the project to the same integer, 1234, we should all get the same sample.  If a student chages that seed or doesn't set it at all, they may get a different sample than the rest of the class.  I didn't ask another student on slack to confirm this.
------------
...
samp2 %>%
  ...
  mutate(p_hat = n /sum(n))
```
By random chance, samp2 is slightly more representative of the population than samp1.  It's proportions are .04% away to the population's proportion whereas samp1's proportion of respondents are .06 away from the population mean.  I think that a sample of 100 responses would be more representative of the population and 1,000 responses would be more accurate still of reflecting the population's proportion of responses.
-------
...
## Answer 4:
There are 15,000 elements if sample_props50.
```{r Answer4}
median(sample_props50$n)
```
The median (center) data point for the distribution is 10 respondents who say they don't benefit from scientific research out of 50 respondents.  As for the graph, it was provided?  So I'm copying and pasting from above?  I don't know if I was supposed to do something different:
```{r Answer4}
ggplot(data = sample_props50, aes(x = p_hat)) +
  geom_histogram(binwidth = 0.02) +
  labs(
    x = "p_hat (Doesn't benefit)",
    title = "Answer 4: Sampling distribution of p_hat",
    subtitle = "Sample size = 50, Number of samples = 15000"
  )
```
...
(sample_props_small <- global_monitor %>%
  ...
  count(scientist_work) %>%
                    ...
                    filter(scientist_work == "Doesn't benefit"))
```
Without the filtering and mutating code, you'd wind up with 250 observations.  This is because each sample has 10 datapoints and you're repeating that sampling process 25 times.  With the filtering and aggregation done here and previously, you wind up with 25 elements each telling you what proportion of that given sample do not believe sicntific research benefits them.
-----------
...
## Answer 6
If you increase either the sampling size or the number of repititions, the curve becomes more normal.  Each data point represents a mean of the the percentage of respondents who think science doesn't beneift them.
...
## Answer 7
    ```{r Answer7}
Answer7 <- global_monitor %>%
                    rep_sample_n(size = 15, reps = 1, replace = TRUE) %>%
                    count(scientist_work) %>%
                    mutate(p_hat = n /sum(n)) %>%
                    filter(scientist_work == "Benefits")
    ```
Using this sample, the best point estimate is 86.7% of respondents think they beneift from scientific research.
...
ggplot(sample_props15, aes(p_hat)) +
  geom_histogram(binwidth = .05)+
  ggtitle("Answer 8")

mean(sample_props15$p_hat)
```
The point estimate for the benefits cohort from this sampe is 8016333.
--------
...
ggplot(sample_props150, aes(p_hat)) +
  geom_histogram(binwidth = .02)+
  ggtitle("Answer 9")

mean(sample_props150$p_hat)
```
The point estimate for the benefits cohort from this sampe is 0.8000233
-----
...
## Answer 10
uhh 2 and 3?  I'm presuming we're referring to questions 8 and 9.  In that case, the one with the sample size of 150 had a much smaller spread over the p_hat variable.  A smaller spread over the p_hat variable is preferrable because you have more confidence in your population mean.
* * *
